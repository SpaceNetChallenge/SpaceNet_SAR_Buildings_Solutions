...
def dpn92_mc(num_classes=1000, num_channels=8):
    model = DPN(
        num_init_features=64, k_r=96, groups=32,
        k_sec=(3, 4, 20, 3), inc_sec=(16, 32, 24, 128),
        num_classes=num_classes, test_time_pool=True, num_channels=num_channels)
    return model
...
class InputBlock(nn.Module):
    def __init__(self, num_init_features, kernel_size=7,
                 padding=3, activation_fn=nn.ReLU(inplace=True), num_channels=3):
        super(InputBlock, self).__init__()
        self.conv = nn.Conv2d(
            num_channels, num_init_features, kernel_size=kernel_size, stride=2, padding=padding, bias=False)
...
class DPN(nn.Module):
    def __init__(self, small=False, num_init_features=64, k_r=96, groups=32,
                 ...
                 num_classes=1000, test_time_pool=False, num_channels=3):
        ...
        # conv1
        if small:
            self.blocks['conv1_1'] = InputBlock(num_init_features, kernel_size=3, padding=1, num_channels=num_channels)
        else:
