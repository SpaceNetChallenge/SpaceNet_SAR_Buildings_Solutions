...
from timm.models.resnet import ssl_resnext50_32x4d
...
from zoo.dpn import dpn92, dpn131, dpn107, dpn92_mc
from zoo.efficient import tf_efficientnet_b5_ns, tf_efficientnet_b7_ns, tf_efficientnet_b4_ns, tf_efficientnet_b3_ns
from zoo.irv import InceptionResNetV2
from zoo.resnet import resnext50_32x4d, resnext101_32x8d, swsl_resnext50_32x4d, swsl_resnext101_32x4d
from zoo.senet import se_resnext50_32x4d, se_resnext101_32x4d, senet154, SCSEModule
...
encoder_params = {
    ...
    'dpn92':
        {'filters': [64, 336, 704, 1552, 2688],
         ...
         'last_upsample': 48,
         'init_op': partial(dpn92_mc, num_channels=4),
         'url': 'http://data.lip6.fr/cadene/pretrainedmodels/dpn92_extra-b040e4a9b.pth'},
    'resnext101':
        {'filters': [64, 256, 512, 1024, 2048],
         'decoder_filters': [64, 192, 256, 256],
         'last_upsample': 64,
         'init_op': partial(swsl_resnext101_32x4d, in_chans=4, pretrained=True),
         'url': None},
    'resnext50':
        {'filters': [64, 256, 512, 1024, 2048],
         'decoder_filters': [64, 128, 256, 256],
         'last_upsample': 64,
         'init_op': partial(swsl_resnext50_32x4d, in_chans=4, pretrained=True),
         'url': None},
    'dpn131':
    ...
    'resnet34':
        {'filters': [64, 64, 128, 256, 512],
         'decoder_filters': [64, 128, 256, 256],
         'last_upsample': 64,
         'init_op': partial(resnet.resnet34, in_channels=4, pretrained=None),
    ...
    'resnet18':
        {'filters': [64, 64, 128, 256, 512],
         ...
         'last_upsample': 48,
         'init_op': partial(resnet.resnet18, in_channels=4, pretrained=None),
    ...
    "tf_efficientnet_b5_ns": {
        ...
        "decoder_filters": [64, 128, 256, 256],
        "init_op": partial(tf_efficientnet_b5_ns, pretrained=True, drop_path_rate=0.2, in_chans=4),
        'url': None,
    },
    "tf_efficientnet_b3_ns": {
        ...
        "decoder_filters": [64, 128, 256, 256],
        "init_op": partial(tf_efficientnet_b3_ns, pretrained=True, drop_path_rate=0.2, in_chans=4),
        'url': None,
    },
    "tf_efficientnet_b4_ns": {
        ...
        "decoder_filters": [64, 128, 256, 256],
        "init_op": partial(tf_efficientnet_b4_ns, pretrained=True, drop_path_rate=0.2, in_chans=4),
        'url': None,
    },
    "tf_efficientnet_b4_ns_7": {
        'last_upsample': 64,
        "filters": [48, 32, 56, 160, 1792],
        "decoder_filters": [64, 128, 256, 256],
        "init_op": partial(tf_efficientnet_b4_ns, pretrained=True, drop_path_rate=0.2, in_chans=7),
        'url': None,
    },
    "tf_efficientnet_b7_ns": {
        ...
        "decoder_filters": [64, 128, 256, 256],
        "init_op": partial(tf_efficientnet_b7_ns, pretrained=True, drop_path_rate=0.2, in_chans=4),
        'url': None,
    },
...
class AbstractModel(nn.Module):
    ...
    def initialize_encoder(self, model, model_url, num_channels_changed=False):
        ...
        if num_channels_changed:
            ...
            model.state_dict()[self.first_layer_params_names[0] + '.weight'][:, 3:4, ...] = pretrained_dict[
                                                                                                self.first_layer_params_names[
                                                                                                    0] + '.weight'].data[
                                                                                            :, 0:1, ...]
...
class SCSeResneXt(EncoderDecoder):
    ...
    def __init__(self, seg_classes, backbone_arch, reduction=2, mode='concat', num_channels=4):
        if not hasattr(self, 'bottleneck_type'):
...
class Resnet(EncoderDecoder):
    def __init__(self, seg_classes, backbone_arch, num_channels=4):
        self.first_layer_stride_two = True,
        super().__init__(seg_classes, num_channels, backbone_arch)
...
class DPNUnet(EncoderDecoder):
    def __init__(self, seg_classes, backbone_arch='dpn92', num_channels=4):
        self.first_layer_stride_two = True
        super().__init__(seg_classes, num_channels, backbone_arch)
...
class EffUnet(EncoderDecoder):
    def __init__(self, seg_classes, backbone_arch='tf_efficientnet_b5_ns'):
        self.first_layer_stride_two = True
        super().__init__(seg_classes, 4, backbone_arch)
...
class IterativeUnet(nn.Module):
    def __init__(self, backbone_func, **kwargs):
        super().__init__()
        self.unet = backbone_func(**kwargs)

    def forward(self, x):
        input = x
        x = torch.cat([input, torch.zeros_like(x[:, :3, ...])], dim=1)
        x = self.unet(x)
        x = torch.sigmoid(x)
...
setattr(sys.modules[__name__], 'iterative_effunet', partial(IterativeUnet, backbone_func=partial(EffUnet)))
...
setattr(sys.modules[__name__], 'eff_unet', partial(EffUnet))
...
if __name__ == '__main__':
    ...
    d = EffUnet(3, backbone_arch="tf_efficientnet_b4_ns_7")
    d.eval()
    with torch.no_grad():
        images = torch.from_numpy(np.zeros((1, 7, 256, 256), dtype="float32"))
        i = d(images)
    print(d)
    print(i.size())
